...
read_when:
  - 运行或配置新手引导
  - 设置新机器
summary: CLI 新手引导：引导式配置 Gateway 网关、工作区、渠道和 Skills
title: 新手引导
x-i18n:
...
新手引导是在 macOS、Linux 或 Windows（通过 WSL2；强烈推荐）上设置 OpenClaw 的**推荐**方式。
它可以在一个引导流程中配置本地 Gateway 网关或远程 Gateway 网关连接，以及渠道、Skills 和工作区默认值。
...
推荐设置 Brave Search API 密钥，以便智能体可以使用 `web_search`（`web_fetch` 无需密钥即可使用）。最简单的配置方式：`openclaw configure --section web`，它会存储密钥在 `tools.web.search.apiKey`。文档：[Web 工具](/tools/web)。
...
1. **现有配置检测**
   ...
   - 重新运行引导**不会**清除任何内容，除非你明确选择**重置**（或传递 `--reset`）。
   - 如果配置无效或包含遗留键名，引导会停止下一步操作并要求你在继续之前运行 `openclaw doctor`。
...
2. **模型/认证**
   ...
   - **Anthropic OAuth（Claude Code CLI）**：在 macOS 上，引导检查钥匙串项目"Claude Code-credentials"（选择"始终允许"以便 launchd 启动不会阻塞）；在 Linux/Windows 上，如果存在则复用 `~/.claude/.credentials.json`。
   - **Anthropic 令牌（粘贴 setup-token）**：在任何机器上运行 `claude setup-token`，然后粘贴令牌（你可以命名它；空白 = 默认）。
   - **OpenAI Code (Codex) 订阅（Codex CLI）**：如果 `~/.codex/auth.json` 存在，引导可以复用它。
...
使用 `openclaw agents add <name>` 创建一个具有独立工作区、会话和认证配置文件的单独智能体。不带 `--workspace` 运行会启动引导。
...
Gateway 网关通过 RPC 暴露引导流程（`wizard.start`、`wizard.next`、`wizard.cancel`、`wizard.status`）。
